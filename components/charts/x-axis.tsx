...
export interface XAxisProps {
  ...
  /** Custom label formatter for tick dates. Falls back to `shortDateFmt` (month + day) when omitted. */
...
function buildDataAlignedTicks({
  ...
  xAccessor,
  xScale,
  formatLabel = (date: Date) => shortDateFmt.format(date),
}: {
  data: Record<string, unknown>[];
  dateLabels: string[];
  marginLeft: number;
  ...
  for (const index of selectEvenlySpacedIndices(data.length, targetTickCount, {
    ...
    // Selection/dedup upstream keys off `dateLabels` (default format); the
    // displayed text can still be overridden via `formatLabel`.
...
function appendProjectionTailTicks(
  ...
  formatLabel: (date: Date) => string = (date: Date) =>
...
const XAxisInner = memo(function XAxisInner({
  ...
  const hoveredLabel =
    isHovering && tooltipData
      ? resolvedFormatLabel(xAccessor(tooltipData.point))
      : null;
