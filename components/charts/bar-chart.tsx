...
const ChartCore = memo(function ChartCore({
  ...
  // Multi-year datasets collide under `shortDateFmt` (e.g. "Jan 2" repeats
  // every year) — that collapses distinct dates onto the same categorical
  // band, silently overlapping bars instead of just duplicating a label.
  // Only include the year when the data actually spans more than one.
  const spansMultipleYears = useMemo(() => {
    ...
    return Number.isFinite(minYear) && maxYear > minYear;
  }, [data, xDataKey]);
    ...
    (d: Record<string, unknown>): string => {
      ...
      if (value instanceof Date) {
        return spansMultipleYears
          ? `${shortDateFmt.format(value)} ${value.getFullYear()}`
          : shortDateFmt.format(value);
      }
      return String(value ?? "");
    },
    [xDataKey, spansMultipleYears]
  );
  ...
  // Create a fake time scale for compatibility with ChartContext. Must be a
  // real callable function (not a plain object) — `XAxis` invokes it as
  // `xScale(date)` whenever its computed domain extends past the last data
  // point (see `domainExtendsPastData` in x-axis.tsx). The domain is built
  // from the REAL first/last bucket dates in `data` (which is time-ordered,
  // same array driving `categoryScale`'s domain) so tick labels reflect the
  // actual data window instead of a `Date.now()`-anchored fake range. Falls
  // back to the old `Date.now()`-based window for empty data, non-Date x
  // values, or a degenerate (identical) start/end date.
  const fakeTimeScale = useMemo(() => {
    const firstValue = data[0]?.[xDataKey];
    const lastValue = data[data.length - 1]?.[xDataKey];
    const hasRealDates =
      firstValue instanceof Date && lastValue instanceof Date;

    const now = Date.now();
    const fallbackStart = now - data.length * 24 * 60 * 60 * 1000;

    let start: number;
    let end: number;
    if (hasRealDates && firstValue.getTime() !== lastValue.getTime()) {
      start = firstValue.getTime();
      end = lastValue.getTime();
    } else {
      start = fallbackStart;
      end = now;
    }

    const scaleFn = ((date: Date) => {
      const t = date.getTime();
      if (end === start) return 0;
      return ((t - start) / (end - start)) * innerWidth;
    }) as unknown as typeof categoryScale & {
      domain: () => Date[];
      range: () => [number, number];
      invert: (x: number) => Date;
      copy: () => typeof scaleFn;
    };
    Object.assign(scaleFn, categoryScale, {
      domain: () => [new Date(start), new Date(end)],
      range: () => [0, innerWidth] as [number, number],
      invert: (x: number) => new Date(start + (x / innerWidth) * (end - start)),
      copy: () => scaleFn,
    });
    return scaleFn;
  }, [categoryScale, innerWidth, data, xDataKey]);
