...
function resolveTimeSeriesYDomain(
  ...
  if (yDomainMode === "data") {
    // Hug the data tightly (price-chart style) instead of pinning to zero —
    // mirrors CandlestickChart's tight min/max/padding scaling.
...
export interface TimeSeriesChartInnerProps {
  ...
  /**
   * `"zero"` (default) pins the domain to `[0, max*1.1]` — good for
   * zero-baseline value series. `"data"` hugs `[min, max]` with ~5% padding
   * like a price chart (mirrors `CandlestickChart`'s tight scaling).
   */
  yDomainMode?: "zero" | "data";
...
const TimeSeriesChartCore = memo(function TimeSeriesChartCore({
  ...
  const resolveYDomain = useCallback(
    ...
    [lines, yScaleDomainMax, yDomainMode]
  );
